      ...
      <article className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="prose prose-invert mt-10 max-w-none text-[15px] leading-7 text-foreground/90">
          ...
          <ol className="mt-3 list-decimal space-y-1 pl-6 text-muted-foreground">
            <li><a href="#background" className="hover:text-primary">背景：金融任务为什么难被单一模型吃下</a></li>
            <li><a href="#architecture" className="hover:text-primary">架构差异:一个大脑 vs 一群专家</a></li>
            <li><a href="#accuracy" className="hover:text-primary">准确率与鲁棒性对比</a></li>
            <li><a href="#explainability" className="hover:text-primary">可解释性与审计追溯</a></li>
            <li><a href="#cost" className="hover:text-primary">成本、延迟与工程复杂度</a></li>
            <li><a href="#compliance" className="hover:text-primary">合规与风险控制</a></li>
            <li><a href="#matrix" className="hover:text-primary">选型对照表</a></li>
            <li><a href="#conclusion" className="hover:text-primary">结论与建议</a></li>
          </ol>

          <h2 id="background" className="mt-12 text-xl font-semibold">1. 背景:金融任务为什么难被单一模型吃下</h2>
          <p className="mt-3">
            金融决策通常同时涉及宏观研判、行业景气、公司基本面、量价与舆情、合规审查等多个视角。
            单一 LLM 会在一次推理中"混合"所有职能——它可能在宏观段落里表现出色,却在合规校验时忽略关键条款。
            当上下文越来越长、任务链越来越复杂,单模型的注意力被稀释,幻觉与"平均化答案"的问题会被放大。
          </p>
          <p className="mt-3">
            这也是 EconSwarm 采用<strong>群智能体(Swarm)</strong>的原因:把宏观、行业、量化、风险、合规拆成
            独立的 Agent,由编排层调度、辩论、投票,再由汇总 Agent 生成可交付结果。
          </p>

          <h2 id="architecture" className="mt-12 text-xl font-semibold">2. 架构差异:一个大脑 vs 一群专家</h2>
          ...
          <h2 id="accuracy" className="mt-12 text-xl font-semibold">3. 准确率与鲁棒性对比</h2>
          <p className="mt-3">
            在需要多源交叉验证的任务里——例如"给出某上市公司近一季度的核心风险"——单一 LLM 常见问题是
            <em>遗漏事实</em>和<em>过度自信</em>。群智能体通过让"看多方"与"看空方"分别举证,再由裁判 Agent 结合数据检索
            打分,能显著降低幻觉。内部基准显示,在结构化投研问答上,群体投票相较单模型可以把关键事实召回率
            提升 20% 以上,同时错误陈述被抓出的概率明显更高。
          </p>

          <h2 id="explainability" className="mt-12 text-xl font-semibold">4. 可解释性与审计追溯</h2>
          <p className="mt-3">
            金融业务对"为什么这么判断"非常敏感。单一 LLM 的推理链条常常是一大段自然语言,难以按角色拆分。
            群智能体的天然优势是:<strong>每个 Agent 的输入、工具调用、结论都独立留痕</strong>,
            审计者可以逐步回放宏观、行业、风险各条链路,合规复核也更容易落地。
          </p>

          <h2 id="cost" className="mt-12 text-xl font-semibold">5. 成本、延迟与工程复杂度</h2>
          <p className="mt-3">
            群智能体不是免费的:并行调用多个模型意味着 token 消耗更高,编排也更复杂。但在实际系统里,
            我们通过<strong>分层模型策略</strong>缓解:低阶 Agent 用轻量模型跑候选,汇总/裁判 Agent 才用高阶模型。
            对复杂投研任务,单一大模型往往需要更长上下文和多轮重试,综合成本反而更高;而对"写一句话摘要"
            这种轻任务,单一 LLM 仍是最经济的选择。
          </p>

          <h2 id="compliance" className="mt-12 text-xl font-semibold">6. 合规与风险控制</h2>
          <p className="mt-3">
            合规 Agent 可以独立配置行业规则(例如信息披露、敏感词、投顾禁区),对其他 Agent 的输出做统一校验,
            并在违规时阻断发布。这种"关卡型 Agent"很难在单一 LLM 的一次推理里稳定实现——
            提示词越长,越容易被前面的内容"带跑"。
          </p>

          <h2 id="matrix" className="mt-12 text-xl font-semibold">7. 选型对照表</h2>
          ...
          <h2 id="conclusion" className="mt-12 text-xl font-semibold">8. 结论与建议</h2>
          <p className="mt-3">
          ...
          <div className="mt-10 rounded-xl border border-subtle bg-section-deep/60 p-6">
            ...
            <div className="mt-2 text-lg font-semibold">在 EconSwarm 工作台里试跑一次多 Agent 投研任务</div>
          ...
          <div className="mt-12 border-t border-subtle pt-6 text-sm text-muted-foreground">
            ...
            <ul className="mt-3 list-disc space-y-1 pl-6">
              <li><Link to="/" hash="principle" className="hover:text-primary">EconSwarm 技术原理:编排 · 辩论 · 汇总</Link></li>
              <li><Link to="/" hash="modules" className="hover:text-primary">四大核心模块:宏观 / 行业 / 量化 / 风险</Link></li>
              <li><Link to="/" hash="capabilities" className="hover:text-primary">群智能体的四项核心能力</Link></li>
            </ul>
          </div>
        </div>
      </article>
